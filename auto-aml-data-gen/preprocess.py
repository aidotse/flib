...
def cal_node_features(df:pd.DataFrame, bank, windows=1) -> pd.DataFrame:
    ...
    for window in windows:
        ...
        df_nodes[f'counts_in_{window[0]}_{window[1]}'] = gb['amount'].apply(lambda x: (x>0).sum())
        df_nodes[f'counts_out_{window[0]}_{window[1]}'] = gb['amount'].apply(lambda x: (x<0).sum())
        df_nodes[f'counts_unique_in_{window[0]}_{window[1]}'] = gb.apply(lambda x: x[x['amount']>0]['counterpart'].nunique())
        df_nodes[f'counts_unique_out_{window[0]}_{window[1]}'] = gb.apply(lambda x: x[x['amount']<0]['counterpart'].nunique())
...
def preprocess(path_to_tx_log:str, banks:list=['defult'], split_type:str='spatial', test_size:float=0.2, overlap:float=0.9):
    ...
    for bank in banks:
        ...
        if split_type == 'temporal':
            ...
            df_nodes = get_nodes(df_bank, bank, windows)
            df_nodes.reset_index(inplace=True)
            df_nodes = df_nodes.sample(frac=1).reset_index(drop=True)
            df_nodes_train = df_nodes.iloc[:int(len(df_nodes)*(1-test_size))]
            df_nodes_test = df_nodes.iloc[int(len(df_nodes)*(1-test_size)):]
            datasets.append((df_nodes_train, df_nodes_test))
    
    return datasets


def preprocess2(path_to_tx_log:str, banks:list=['defult'], train_test_overlap:float=0.9):
