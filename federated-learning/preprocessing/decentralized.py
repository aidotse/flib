...
def main():
    ...
    for bank in banks:
        ...
        num_cpus = 8
        accounts_list = np.array_split(accounts, num_cpus)
        ...
        #        'account': account, 
        #        'num_outgoing_txs_1': num_txs(df_bank, account, ranges[0]), 'num_outgoing_txs_2': num_txs(df_bank, account, ranges[1]), 'num_outgoing_txs_3': num_txs(df_bank, account, ranges[2]),
        #        'sum_outgoing_txs_1': sum_txs(df_bank, account, ranges[0]), 'sum_outgoing_txs_2': sum_txs(df_bank, account, ranges[1]), 'sum_outgoing_txs_3': sum_txs(df_bank, account, ranges[2]),
        #        'freq_outgoing_txs_1': freq_txs(df_bank, account, ranges[0]), 'freq_outgoing_txs_2': freq_txs(df_bank, account, ranges[1]), 'freq_outgoing_txs_3': freq_txs(df_bank, account, ranges[2]),
        #        'num_txs_1': num_txs(df_bank, account, ranges[0]), 'num_txs_2': num_txs(df_bank, account, ranges[1]), 'num_txs_3': num_txs(df_bank, account, ranges[2]),
        #        'sum_txs_1': sum_txs(df_bank, account, ranges[0]), 'sum_txs_2': sum_txs(df_bank, account, ranges[1]), 'sum_txs_3': sum_txs(df_bank, account, ranges[2]),
        #        'freq_txs_1': freq_txs(df_bank, account, ranges[0]), 'freq_txs_2': freq_txs(df_bank, account, ranges[1]), 'freq_txs_3': freq_txs(df_bank, account, ranges[2]),
        #        'num_unique_counterparties_1': num_unique_counterparties(df_bank, account, ranges[0]), 'num_unique_counterparties_2': num_unique_counterparties(df_bank, account, ranges[1]), 'num_unique_counterparties_3': num_unique_counterparties(df_bank, account, ranges[2]),
        #        'freq_unique_counterparties_1': freq_unique_counterparties(df_bank, account, ranges[0]), 'freq_unique_counterparties_2': freq_unique_counterparties(df_bank, account, ranges[1]), 'freq_unique_counterparties_3': freq_unique_counterparties(df_bank, account, ranges[2]),
        #        'num_phone_changes_1': num_phone_changes(df_bank, account, ranges[0]), 'num_phone_changes_2': num_phone_changes(df_bank, account, ranges[1]), 'num_phone_changes_3': num_phone_changes(df_bank, account, ranges[2]),
        #        'freq_phone_changes_1': freq_phone_changes(df_bank, account, ranges[0]), 'freq_phone_changes_2': freq_phone_changes(df_bank, account, ranges[1]), 'freq_phone_changes_3': freq_phone_changes(df_bank, account, ranges[2]),
        #        'num_bank_changes_1': num_days_in_bank(df_bank, account, ranges[0]), 'num_bank_changes_2': num_days_in_bank(df_bank, account, ranges[1]), 'num_bank_changes_3': num_days_in_bank(df_bank, account, ranges[2]),
        #        'freq_bank_changes_1': freq_days_in_bank(df_bank, account, ranges[0]), 'freq_bank_changes_2': freq_days_in_bank(df_bank, account, ranges[1]), 'freq_bank_changes_3': freq_days_in_bank(df_bank, account, ranges[2]),
        #        'is_sar': is_sar(df_bank, account)
        #    }, ignore_index=True)
        #processed_df.to_csv(f'/home/edvin/Desktop/flib/federated-learning/datasets/{DATASET}/{bank}.csv', index=False)
