...
def cal_node_features(df:pd.DataFrame, bank) -> pd.DataFrame:
    ...
    df_nodes = pd.concat([banks, sums, means, medians, stds, maxs, mins, counts_in, counts_out, counts_days_in_bank, counts_phone_changes, sums_spending, means_spending, medians_spending, stds_spending, maxs_spending, mins_spending, counts_spending, is_sar], axis=1)
...
def main():
    ...
    df = load_data(path)
    banks = ['bank'] #set(df['bankOrig'].unique().tolist() + df['bankDest'].unique().tolist())
