# class Server_old():
#     def __init__(self, seed, clients, n_workers):
#         self.seed = seed
#         self.clients = clients
#         self.n_workers = n_workers
        
#     def _train_clients(self, clients, seed):
#         state_dicts = []
#         client_names = []
#         losses = []
...
#             losses.append(loss)
#         return client_names, losses, state_dicts
...
#         results_dict = {client.name: {"train": {}, "val": {}, "test": {}} for client in self.clients}
...
#                 for client, loss, y_pred, y_true in zip(result[0], result[1], result[2], result[3]):
#                     results_dict[client]["train"]["round"] = [0] 
#                     results_dict[client]["train"]["accuracy"] = [accuracy_score(y_true, (y_pred[:,1] > 0.5))] 
#                     results_dict[client]["train"]["average_precision"] = [average_precision_score(y_true, y_pred[:,1], recall_span=(0.6, 1.0))] 
#                     results_dict[client]["train"]["balanced_accuracy"] = [balanced_accuracy_score(y_true, (y_pred[:,1] > 0.5))] 
#                     results_dict[client]["train"]["f1"] = [f1_score(y_true, (y_pred[:,1] > 0.5), pos_label=1, zero_division=0.0)] 
#                     results_dict[client]["train"]["loss"] = [loss] 
#                     results_dict[client]["train"]["precision"] = [precision_score(y_true, (y_pred[:,1] > 0.5), pos_label=1, zero_division=0.0)] 
#                     results_dict[client]["train"]["recall"] = [recall_score(y_true, (y_pred[:,1] > 0.5), pos_label=1, zero_division=0.0)] 
...
#                         results_dict[client]["val"]["round"] = [0] 
#                         results_dict[client]["val"]["accuracy"] = [accuracy_score(y_true, (y_pred[:,1] > 0.5))] 
#                         results_dict[client]["val"]["average_precision"] = [average_precision] 
#                         results_dict[client]["val"]["balanced_accuracy"] = [balanced_accuracy_score(y_true, (y_pred[:,1] > 0.5))] 
#                         results_dict[client]["val"]["f1"] = [f1_score(y_true, (y_pred[:,1] > 0.5), pos_label=1, zero_division=0.0)] 
#                         results_dict[client]["val"]["loss"] = [loss] 
#                         results_dict[client]["val"]["precision"] = [precision_score(y_true, (y_pred[:,1] > 0.5), pos_label=1, zero_division=0.0)] 
#                         results_dict[client]["val"]["recall"] = [recall_score(y_true, (y_pred[:,1] > 0.5), pos_label=1, zero_division=0.0)] 
#                         previous_val_average_precision += average_precision / len(self.clients)
            
#             for round in tqdm(range(1, n_rounds+1), desc='progress', leave=False):
                
#                 results = p.starmap(self._train_clients, [(client_split, self.seed+round) for client_split in client_splits])
#                 state_dicts = []
#                 for result in results:
#                     for client, loss, state_dict in zip(result[0], result[1], result[2]):
...
#                 for result in results:
#                     for client, loss, y_pred, y_true in zip(result[0], result[1], result[2], result[3]):
#                         results_dict[client]["train"]["round"].append(round)
#                         results_dict[client]["train"]["accuracy"].append(accuracy_score(y_true, (y_pred[:,1] > 0.5)))
#                         results_dict[client]["train"]["average_precision"].append(average_precision_score(y_true, y_pred[:,1], recall_span=(0.6, 1.0)))
#                         results_dict[client]["train"]["balanced_accuracy"].append(balanced_accuracy_score(y_true, (y_pred[:,1] > 0.5)))
#                         results_dict[client]["train"]["f1"].append(f1_score(y_true, (y_pred[:,1] > 0.5), pos_label=1, zero_division=0.0))
#                         results_dict[client]["train"]["loss"].append(loss)
#                         results_dict[client]["train"]["precision"].append(precision_score(y_true, (y_pred[:,1] > 0.5), pos_label=1, zero_division=0.0))
#                         results_dict[client]["train"]["recall"].append(recall_score(y_true, (y_pred[:,1] > 0.5), pos_label=1, zero_division=0.0))
...
#                     results = p.starmap(self._evaluate_clients, [(client_split, 'valset') for client_split in client_splits])
#                     avg_val_average_precision = 0.0
#                     for result in results:
#                         for client, loss, y_pred, y_true in zip(result[0], result[1], result[2], result[3]):
#                             val_average_precision = average_precision_score(y_true, y_pred[:,1], recall_span=(0.6, 1.0))
#                             results_dict[client]["val"]["round"].append(round)
#                             results_dict[client]["val"]["accuracy"].append(accuracy_score(y_true, (y_pred[:,1] > 0.5)))
#                             results_dict[client]["val"]["average_precision"].append(val_average_precision)
#                             results_dict[client]["val"]["balanced_accuracy"].append(balanced_accuracy_score(y_true, (y_pred[:,1] > 0.5)))
#                             results_dict[client]["val"]["f1"].append(f1_score(y_true, (y_pred[:,1] > 0.5), pos_label=1, zero_division=0.0))
#                             results_dict[client]["val"]["loss"].append(loss)
#                             results_dict[client]["val"]["precision"].append(precision_score(y_true, (y_pred[:,1] > 0.5), pos_label=1, zero_division=0.0))
#                             results_dict[client]["val"]["recall"].append(recall_score(y_true, (y_pred[:,1] > 0.5), pos_label=1, zero_division=0.0))
#                             avg_val_average_precision += val_average_precision / len(self.clients)
...
#                 for result in results:
#                     for client, loss, y_pred, y_true in zip(result[0], result[1], result[2], result[3]):
#                         results_dict[client]["train"]["precision_recall_curve"] = precision_recall_curve(y_true, y_pred[:,1])
#                         results_dict[client]["train"]["roc_curve"] = roc_curve(y_true, y_pred[:,1])
#                 results = p.starmap(self._evaluate_clients, [(client_split, 'valset') for client_split in client_splits])
#                 for result in results:
#                     for client, loss, y_pred, y_true in zip(result[0], result[1], result[2], result[3]):
#                         results_dict[client]["val"]["precision_recall_curve"] = precision_recall_curve(y_true, y_pred[:,1])
#                         results_dict[client]["val"]["roc_curve"] = roc_curve(y_true, y_pred[:,1])
#                 results = p.starmap(self._evaluate_clients, [(client_split, 'testset') for client_split in client_splits])
#                 for result in results:
#                     for client, loss, y_pred, y_true in zip(result[0], result[1], result[2], result[3]):
#                         results_dict[client]["test"]["round"] = [round]
#                         results_dict[client]["test"]["accuracy"] = [accuracy_score(y_true, (y_pred[:,1] > 0.5))]
#                         results_dict[client]["test"]["average_precision"] = [average_precision_score(y_true, y_pred[:,1], recall_span=(0.6, 1.0))]
#                         results_dict[client]["test"]["balanced_accuracy"] = [balanced_accuracy_score(y_true, (y_pred[:,1] > 0.5))]
#                         results_dict[client]["test"]["f1"] = [f1_score(y_true, (y_pred[:,1] > 0.5), pos_label=1, zero_division=0.0)]
#                         results_dict[client]["test"]["loss"] = [loss]
#                         results_dict[client]["test"]["precision_recall_curve"] = precision_recall_curve(y_true, y_pred[:,1])
#                         results_dict[client]["test"]["precision"] = [precision_score(y_true, (y_pred[:,1] > 0.5), pos_label=1, zero_division=0.0)]
#                         results_dict[client]["test"]["recall"] = [recall_score(y_true, (y_pred[:,1] > 0.5), pos_label=1, zero_division=0.0)]
#                         results_dict[client]["test"]["roc_curve"] = roc_curve(y_true, y_pred[:,1])
#         return results_dict
...
class TorchServer():
    def __init__(self, seed: int, device: int, Model: Any, optimizer: str, clients: List, n_workers: int = None, **kwargs):
        self.seed = seed
        self.device = device
